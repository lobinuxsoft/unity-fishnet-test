...
namespace FishNet.Discovery
{
	...
	public sealed class NetworkDiscovery : MonoBehaviour
	{
		...
		private void LogInformation(string message) => _networkManager.Log($"[{nameof(NetworkDiscovery)}] {message}");

		/// <summary>
		/// Logs a warning if the NetworkManager can log.
		/// </summary>
		/// <param name="message">Message to log.</param>
		private void LogWarning(string message) => _networkManager.LogWarning($"[{nameof(NetworkDiscovery)}] {message}");

		/// <summary>
		/// Logs an error if the NetworkManager can log.
		/// </summary>
		/// <param name="message">Message to log.</param>
		private void LogError(string message) => _networkManager.LogError($"[{nameof(NetworkDiscovery)}] {message}");
	}
}
